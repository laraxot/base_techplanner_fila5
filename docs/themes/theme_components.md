...
## ⚠️ REGOLA CRITICA: Validazione Dati nei Temi

I temi devono rimanere “presentazione pura”.

- **[NO runtime guards in Blade]** Non usare `Webmozart\Assert\Assert` direttamente nelle view Blade dei temi.
- **[Validare a monte]** Se una view di tema riceve un payload `$data` (es. da widget/modulo), la validazione va fatta nel livello modulo (widget/controller/action) usando runtime guards (`Assert::...`).

Motivazione:

- Il tema deve essere sostituibile senza cambiare logica.
- La validazione deve stare nel punto dove i dati vengono prodotti/normalizzati.
