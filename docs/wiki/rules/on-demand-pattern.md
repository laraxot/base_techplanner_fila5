...
```text
Session start (unico preload)
    -> docs/wiki/concepts/agent-bootstrap-compact.md  # ≤100 righe, STORY-150

Task triggered
    -> consult docs/wiki/rules/00-TRIGGER_MAP.md (1 riga dominio)
    -> qmd search "<topic>" --limit 5
    -> read only matched wiki files (max 5)
...
**Preload unico agente:** `docs/wiki/concepts/agent-bootstrap-compact.md` (< 120 righe). Contiene workflow, religioni, token, trigger rapidi.

Stubs (`CLAUDE.md`, `QWEN.md`, `GEMINI.md`, `AGENTS.md`) must be <=50 lines — link al compact + trigger map, no regole embeddate.

Memoria User Rules: `docs/wiki/memories/agent-token-bootstrap-slim.md` (≤12 righe incollabili).
...
- [Agent bootstrap compatto](../concepts/agent-bootstrap-compact.md)
