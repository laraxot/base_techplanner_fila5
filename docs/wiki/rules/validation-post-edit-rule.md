...
**Smoke minimo** se non c’è suite E2E sul path toccato:
...
```bash
cd laravel
php artisan test Modules/<Module>/tests   # o --testsuite=<Name> se registrato in phpunit.xml
```

Documentare exit code e sintesi. Se la suite è bloccata da errori **preesistenti** (es. `TestClosureMustNotBeStatic`), riportare path e messaggio — il tentativo resta obbligatorio.

## 3c. Chiusura task — vietato «dovrebbe»

Non dichiarare completato senza output empirico. Memoria: [`agent-verification-mandatory-no-dovrebbe.md`](../memories/agent-verification-mandatory-no-dovrebbe.md).
