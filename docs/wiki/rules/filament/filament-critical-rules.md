...
## Regola Critica Aggiuntiva: Superfici List-Like Pubbliche = Table Widget Filament

### Ambito
Front office pubblico, con priorita al modulo Predict

### Regola
Se una sezione pubblica mostra una lista esplorabile di record o outcomes con bisogno di ricerca, filtri, ordinamento o paginazione, il renderer corretto e un `XotBaseTableWidget` o un widget table Filament equivalente.

### Vale anche dentro una detail page
- Detail page shell, hero, chart, order book, trading form: Blade o widget custom
- Sezione list-like dentro il detail, per esempio outcomes di un mercato multi-outcome: widget table Filament

### Anti-pattern
- `@foreach` Blade per outcomes con filtro/ordinamento implicito o assente
- array preparati in widget e poi riesplosi in Blade per simulare una tabella
- search/filter/sort ricostruiti manualmente nel template

### Pattern corretto
- query nel widget
- colonne, filtri e sort nel widget
- Blade come shell o bridge minimo via `@livewire(...)`
