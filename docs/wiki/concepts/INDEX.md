---
title: "Concepts Index"
type: "index"
tags: [concepts, architecture, patterns]
module: "root"
---

# Concepts — Root Wiki

> Architectural concepts, patterns, and design decisions.

## Available Concepts

- [module-structure](./module-structure.md)
- [actions-over-services](./actions-over-services.md)
- [accessor-auto-persistence](./accessor-auto-persistence.md)
- [architecture-guardrails](./architecture-guardrails.md)
- [ai-tooling-workflow](./ai-tooling-workflow.md)
- [bmad-operating-model](./bmad-operating-model.md)
- [second-brain-operating-model](./second-brain-operating-model.md)
- [second-brain-continuous-improvement](./second-brain-continuous-improvement.md)
- [second-brain-audit-checks](./second-brain-audit-checks.md)
- [second-brain-maintenance-cadence](./second-brain-maintenance-cadence.md)
- [context-mode-plugin](./context-mode-plugin.md)
- [context-mode-cli-reference](./context-mode-cli-reference.md)
- [spatie-permission-teams-laravel-13](./spatie-permission-teams-laravel-13.md)
- [laravel13-modular-composer-upgrade](./laravel13-modular-composer-upgrade.md)
- [llm-wiki-operational-discipline](./llm-wiki-operational-discipline.md)
- [hackernoon-ai-coding-tips-fixcity-map](./hackernoon-ai-coding-tips-fixcity-map.md)
- [markdown-note-minimum-standard](./markdown-note-minimum-standard.md)
- [context-overflow-prevention](./context-overflow-prevention.md)
- [cms-x-page-opaque-data-bag](./cms-x-page-opaque-data-bag.md)
- [filament-primary-color-pa](./filament-primary-color-pa.md)
- [login-page-ux-audit](./login-page-ux-audit.md)

## Usage

```bash
qmd search "concept:<topic>" --limit 5
```

---

**Upstream:** [Trigger Map](../rules/00-TRIGGER_MAP.md)
