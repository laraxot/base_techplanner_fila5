...
class BBjson
{
    ...
    */
    ...
    public function init($payload)
    {
        ...
        if (0 === count($this->payload['push']['changes'])) {
    ...
    private function load_config()
    {
        if (! is_file('config.php')) {
            $this->error('Can\'t find config.php ['.__LINE__.']');
        }
        ...
        foreach ($config['repos'] as $repo) {
            if (false !== strpos($payload_repo_name, $repo['repo_name']) && 'bitbucket' == $repo['repo_host']) {
        ...
        foreach ($this->payload['push']['changes'] as $change) {
            ...
            $this->config->branch[$branch_name]['changes'] = [];
    ...
    private function load_changed_files($branch_config)
    {
        ...
        foreach ($branch_config['changes'] as $index => $change) {
            ...
            do {
                ...
                $file_path = 'removed' == $file['status'] ? $file['old']['path'] : $file['new']['path'];
                ...
                if ('removed' == $file['status']) {
                    ...
                    if (429 == $http_code) {
                        ...
                        if (file_put_contents($wrapper.$file_path, $data, 0, stream_context_create(['ftp' => ['overwrite' => true]]))) {
