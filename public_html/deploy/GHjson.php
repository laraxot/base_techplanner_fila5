...
class GHjson
{
    ...
    public function init($pl)
    {
        ...
        $this->load_datas();
        // $this->log_it(print_r($this->data, true));
        $this->log_it('line:'.__LINE__);
        foreach ($this->data->branch as $br) {
            $this->load_files($br);
        }
    }

    public function load_files($br)
    {
        $ftp = $br['ftp'];

        if ('ssh' == $ftp['type'] && ! function_exists('ssh2_connect')) {
            $this->error('error: You don\'t have SSH capabilities on this server. You must install the SSH2 extension available from PECL');
        }

        // Makes a nice path
        if ('/' != substr($ftp['ftp_path'], 0, 1) && 'none' != $ftp['type']) {
            $ftp['ftp_path'] = '/'.$ftp['ftp_path'];
        }
        if ('/' != substr($ftp['ftp_path'], strlen($ftp['ftp_path']) - 1, 1)) {
            $ftp['ftp_path'] = $ftp['ftp_path'].'/';
        }
        ...
        foreach ($br['commits'] as $commit) {
            ...
            foreach ($files as $file) {
                ...
                if (200 != $http_code) {
                    ...
                    if (file_put_contents($wrapper.$file, $data, 0, stream_context_create(['ftp' => ['overwrite' => true]]))) {
    ...
    public function load_datas()
    {
        ...
        //    $this->error('Can\'t find config.php ['.__LINE__.'] filesize '.filesize('config.php').' file_exists '.file_exists('config.php'));
        // }
        ...
        foreach ($config['repos'] as $repo) {
            if (strtolower($pl_repo_name) == strtolower($repo['repo_name']) && 'github' == $repo['repo_host']) {
        ...
        $this->data->branch[$branch_name]['commits'] = [];
    ...
    */
    ...
    public function log_it($text, $save = true)
    {
    ...
    public function log_payload($text)
    {
