...
\`\`\`bash
qmd search "<topic>" -c notify
# Cerca solo in ./laravel/Modules/Notify/docs/wiki/
\`\`\`
...
\`\`\`bash
qmd search "form" -c notify  # Solo questo modulo
\`\`\`
...
| Ricerca lenta | Limita con `-c notify` |
