...
class EditRole extends XotBaseEditRecord
{
    ...
    public function afterSave(): void
    {
        ...
        $this->permissions->each(static function (mixed $permission) use ($permissionModels, $data): void {
    ...
    protected function mutateFormDataBeforeSave(array $data): array
    {
        $this->permissions = collect($data)
            ->filter(
                static fn (mixed $_permission, string $key): bool => ! \in_array($key, ['name', 'guard_name', 'select_all'], false) && Str::contains($key, '_'),
            )
            ->keys();
