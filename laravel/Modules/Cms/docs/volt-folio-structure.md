...
- ✅ `<x-layouts.main>` - Layout HTML di base (wrapper `<!DOCTYPE html>`, `<html>`, `<body>`). Viene esteso da `x-layouts.app` e `x-layouts.guest` e **non** dovrebbe essere usato direttamente nelle pagine Folio.
- ✅ `<x-layouts.app>` - Layout principale dell'applicazione (header + navigation + footer).
- ✅ `<x-layouts.guest>` - Layout per pagine guest (login, registrazione, recupero password, ecc.).
