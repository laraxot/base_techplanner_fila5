 ...
 * sotto questa stessa cartella; i rimanenti sono mantenuti qui per
 * retro-compatibilità finché non vengono suddivisi.
 */
class FileAction
{
    use QueueableAction;
...
>>>>>>> 7f6cf6be (.)
        ...
        Assert::isMap($data, '['.__LINE__.']['.class_basename(self::class).']');
...
>>>>>>> 7f6cf6be (.)
            ...
            return @round($bytes / 1024 ** ($i = (int) floor(log($bytes, 1024))), 2).' '.($unit[$i] ?? 'B');
        }
...
>>>>>>> 7f6cf6be (.)
        ...
        return @round($bytes / 1000 ** ($i = (int) floor(log($bytes, 1000))), 2).' '.($unit[$i] ?? 'B');
    }
...
>>>>>>> 7f6cf6be (.)
    ...
    {
        // Action di raccolta: i metodi statici possono essere usati come utility
        // retro-compatibili finché non vengono estratti in action dedicate.
    }
}
