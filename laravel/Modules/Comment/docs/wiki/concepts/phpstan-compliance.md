---
title: "Comment Module - PHPStan Type Compliance"
type: concept
tags: [comment, phpstan, types, compliance, quality, static-analysis]
created: 2026-06-10
updated: 2026-06-10
related:
...
# Comment Module — PHPStan Type Compliance
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Comment \
  --level=max \
  --no-progress
```
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Comment --level=max
```
...
# Verbose mode
vendor/bin/phpstan analyse laravel/Modules/Comment --level=max -v
```
...
```bash
vendor/bin/pest laravel/Modules/Comment/tests --parallel
```
