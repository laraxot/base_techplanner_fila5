...
- **Critical Boundary**: The **User** module must **NEVER** depend on the **Comment** module. This prevents circular dependencies and fatal errors (e.g., when the Comment module is absent). Core User models (like `BaseUser`) must not use traits from the Comment module (e.g., `InteractsWithComments`, `CanComment`).
